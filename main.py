...
class Tearapy(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for page in (StartPage, Chat, Stats, Settings):
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller): #read on deacoupling the pages
        ...
        text = Label(self, text = "Tearapy")
        text.pack(side="top", fill="x", pady=10)

        #TODO: how to include emojis here

        button_chat = Button(self, text = "Chat", command = lambda: controller.show_frame("Chat"))
        button_chat.pack()

        button_stats = Button(self, text = "Stats", command = lambda: controller.show_frame("Stats"))
        button_stats.pack()

        button_sets = Button(self, text = "Sets", command = lambda: controller.show_frame("Settings"))
        button_sets.pack()
